...
# NLL-selected lr per model (1-epoch, 256-sel), locked 2026-08-04
CHOSEN_LR = {"olmo1b": "1e-3", "qwen7b": "1e-4", "llama8b": "3e-4",
...
def dpo_cell(mtag, lr):
    base = _salve_run(mtag, lr)   # prefer n_val_sel=256 re-readout if present
...
def beh_cell(mtag, lr, metric):
    base = _salve_run(mtag, lr)   # prefer 256 behavioral, fall back to 128
    for name in (f"{base}_n256", base):
        e = _judged(BEHROOT / f"beh_{name}" / "judged_scores.json", "salve")
        if e and e.get(metric) is not None:
            return e.get(metric)
    return None
...
def main():
    ...
    for ci, mtag in enumerate(MODELS):
        ...
        # star the NLL-chosen lr on the verbalized point + faint vline across rows
        clr = CHOSEN_LR.get(mtag)
        if clr and clr in LRS:
            for ri in range(3):
                axes[ri][ci].axvline(LRX[clr], color="goldenrod", lw=1.1,
                                     alpha=0.35, zorder=0)
            cr = dpo_cell(mtag, clr)
            if cr and cr["loss"] is not None:
                ax.plot(LRX[clr], cr["loss"], marker="*", ms=20, color="gold",
                        mec="k", mew=0.6, zorder=6, label="chosen lr")
