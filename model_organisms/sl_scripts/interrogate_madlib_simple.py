"""Train a mad-lib soft prompt on SL:cat with a *minimal* 2-hole scaffold:

    "You are <8>. You <8>."

Just 16 learnable embeddings (vs 48 in interrogate_madlib.py, vs 128 in
interrogate_soft.py). Tests how much capacity is actually needed to fit
the distillation, and whether the simpler structure makes per-hole decoding
more legible.
...
CKPT_PATH = Path("/nlp/scr/nathu/latent_rewrite/results/model_organisms/"
                 "interrogate_madlib_simple_sl_cat.pt")
...
SCAFFOLD = f"You are {_HOLE}. You {_HOLE}."
SLOT_SIZE = 8
...
#%% sentence-by-sentence interrogation: ask the soft-prompted model to
# verbalize what fills each blank — exploits the scaffold's structural prior
# to probe one role at a time.
sentence_probes = [
    ("You are ___",
     "Looking at the first sentence of your system prompt — 'You are ___' "
     "— what fills the blank? Answer in one short phrase."),
    ("You ___",
     "Your system prompt's second sentence is 'You ___'. What fills the "
